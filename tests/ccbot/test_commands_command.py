...
from ccbot.cc_commands import CCCommand
...
class TestCommandsCommand:
    ...
    async def test_no_discoverable_commands_reports_provider(self) -> None:
        ...
        with (
            patch("ccbot.bot.session_manager") as mock_sm,
            patch("ccbot.bot.get_provider_for_window", return_value=provider),
            patch("ccbot.bot._sync_scoped_provider_menu", new_callable=AsyncMock),
            patch("ccbot.bot.discover_provider_commands", return_value=[]),
            patch("ccbot.bot.safe_reply", new_callable=AsyncMock) as mock_reply,
        ):
    ...
    async def test_lists_provider_commands_with_original_mapping(self) -> None:
        ...
        with (
            ...
            patch("ccbot.bot.get_provider_for_window", return_value=provider),
            patch(
                "ccbot.bot._sync_scoped_provider_menu", new_callable=AsyncMock
            ) as mock_sync,
            patch("ccbot.bot.discover_provider_commands", return_value=discovered),
            patch("ccbot.bot.safe_reply", new_callable=AsyncMock) as mock_reply,
        ):
            mock_sm.resolve_window_for_thread.return_value = "@1"
            await commands_command(update, MagicMock())
...
class TestScopedProviderMenuSync:
    async def test_caches_provider_menu_per_chat_user(self) -> None:
        ...
        try:
            ...
            with patch(
                "ccbot.bot.register_commands", new_callable=AsyncMock
            ) as mock_reg:
    ...
    async def test_scoped_menu_cache_is_bounded(self) -> None:
        ...
        try:
            ...
            with (
                patch("ccbot.bot._MAX_SCOPED_PROVIDER_MENU_ENTRIES", 1),
                patch("ccbot.bot.register_commands", new_callable=AsyncMock),
            ):
                await _sync_scoped_provider_menu(message, 100, provider)
